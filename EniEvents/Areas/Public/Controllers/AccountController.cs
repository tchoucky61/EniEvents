...
namespace EniEvents.Areas.Public.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            repoUtilisateur = new UtilisateurRepository(dbContext);
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    UserManager.AddToRole(user.Id, UserRoles.ROLE_USER);
                    await SignInManager.SignInAsync(user, isPersistent:false, rememberBrowser:false);

                    Utilisateur utilisateur = model.Utilisateur;
                    utilisateur.ApplicationUserId = user.Id;
                    repoUtilisateur.Insert(utilisateur);
